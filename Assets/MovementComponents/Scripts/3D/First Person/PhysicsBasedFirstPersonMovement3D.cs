namespace DUJAL.MovementComponents.PhysicsBased3D 
{
    ...
    public class PhysicsBasedFirstPersonMovement3D : MonoBehaviour
    {
        ...
        [SerializeField] [Range(0, 50)] private float _slideForce;
        ...
        [SerializeField] [Range(0, 10)] private float _speedIncreaseMultiplier;
        [SerializeField] [Range(0, 10)] private float _slopeIncreaseMultiplier;
        ...
        private void Update()
        {
            Debug.Log(new Vector3(_rigidbody.velocity.x, 0f, _rigidbody.velocity.z).magnitude);
            HandleSates();
            HandleDrag();
            HandleDeviceChange();
        }
        ...
        {
            if (IsGrounded) _rigidbody.drag = _groundDrag;
            else _rigidbody.drag = _airDrag;
        }

        private void CheckGrounded()
        {
            IsGrounded = Physics.Raycast(transform.position, Vector3.down, _playerHeight * 0.5f + 0.2f, _groundCollisionLayer);
        }

        private void HandleInput()
        {
            _movementMap = new MovementInput();
            _movementMap.FPS.Enable();

            _movementMap.FPS.Move.performed += ctx => { PerformMovement(ctx.ReadValue<Vector2>()); };
            _movementMap.FPS.Move.canceled += ctx => { PerformMovement(ctx.ReadValue<Vector2>()); };

            _movementMap.FPS.CameraMovement.performed += ctx => { PerformCameraMovement(ctx.ReadValue<Vector2>()); };
            _movementMap.FPS.CameraMovement.canceled += ctx => { PerformCameraMovement(ctx.ReadValue<Vector2>()); };

            _movementMap.FPS.Crouch.performed += ctx => { PerformCrouch(); };
            _movementMap.FPS.Crouch.canceled += ctx => { CancelCrouch(); };
            
            _movementMap.FPS.Slide.performed += ctx => { PerformSlide(); };
            _movementMap.FPS.Slide.canceled += ctx => { CancelSlide(); };
        }
        ...
        private void PerformCameraMovement(Vector2 mouseDelta)
        {
            ...
            _orientation.rotation = Quaternion.Euler(0, _cameraRotation.y, 0);
        }
        ...
        private void SetSlidingState() 
        {
            ...
            if (CheckSlope() && _rigidbody.velocity.y < 0.1f) _desiredMoveSpeed = _slideForce;
        ...
        {
            if (_movementMap.FPS.Jump.IsPressed() && _readyToJump && IsGrounded)
            {
                _readyToJump = false;
        ...
        private void HandleSates()
        {
            ...
            else if (_movementMap.FPS.Run.IsPressed() && IsGrounded && !IsCrouching)
            {
                SetRunState();
            }
            ...
            if (Mathf.Abs(_desiredMoveSpeed - _lastDesiredMoveSpeed) > 4f && _currentMoveSpeed != 0) 
            {
        ...
        private IEnumerator SmoothlyLerpMoveSpeed()
        {
            ...
            while (timer < delta)
            {
                ...
                if (CheckSlope())
                {
                    ...
                    timer += Time.deltaTime * _speedIncreaseMultiplier * _slopeIncreaseMultiplier * slopeAngleIncrease;
                }
                else
                    timer += Time.deltaTime * _speedIncreaseMultiplier;
        ...
        {
            _moveDir = _orientation.forward * _movementInput.y + _orientation.right * _movementInput.x;
